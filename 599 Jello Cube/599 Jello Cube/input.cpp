...
void writeWorld (char * fileName, struct world * jello)
{
  ...
  jello->kElastic=575;
  jello->dElastic=0.25;
  jello->kCollision=100.0;
  jello->dCollision=0.7;		// bigger the value lesser the rebound force
  jello->mass= 1.0 / 512;
    ...
    for (j=0; j<= jello->resolution-1; j++)
      for (k=0; k<= jello->resolution-1; k++)
      {
        ...
        jello->forceField[i * jello->resolution * jello->resolution 
          + j * jello->resolution + k].x = 10.5432; 
        jello->forceField[i * jello->resolution * jello->resolution 
          + j * jello->resolution + k].y = -10.6578;
        jello->forceField[i * jello->resolution * jello->resolution 
          + j * jello->resolution + k].z = 30.321;
      }
  ...
  for (i=0; i<=7; i++)
    for (j=0; j<=7; j++)
	    for (k=0; k<=7; k++)
      {
        ...
        if ((i==7) && (j==7) && (k==7))
        {
          jello->p[i][j][k].x=1.0 + 0.5 / 7;//1.0 + 1.0 / 7;
    	  jello->p[i][j][k].y=1.0 + 0.3 / 7;
	      jello->p[i][j][k].z=1.0 + 0.7 / 7;
        }
  ...
  for (i=0; i<=7; i++)
    for (j=0; j<=7; j++)
      for (k=0; k<=7; k++)
      {
        jello->v[i][j][k].x=-30.0;
	  	  jello->v[i][j][k].y=35.0;
		    jello->v[i][j][k].z=-51.0;
      }
